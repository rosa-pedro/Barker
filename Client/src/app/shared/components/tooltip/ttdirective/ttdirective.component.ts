...
export class TtdirectiveComponent {
  ...
  create() {
    ...
    this.renderer.appendChild(
      this.tooltip,
      this.renderer.createText(this.data) // textNode
    );
  ...
  setPosition() {
    // 호스트 요소의 사이즈와 위치 정보
    const hostPos = this.ele.nativeElement.getBoundingClientRect();
  ...
  private openTooltip() {
    let tooltip = this.viewContainerRef.createComponent(TooltipComponent);

    tooltip.instance.left = Math.round(
      this.ele.nativeElement.getBoundingClientRect().left +
        this.ele.nativeElement.offsetWidth / 2
    );
    tooltip.instance.top =
      this.ele.nativeElement.getBoundingClientRect().top +
